...
export default function Admin() {
  ...
  const fiteredProducts = products.filter((p) => (p?.animal === selectedAnimal || selectedAnimal === "all")).filter((p) => (p?.categoria === selectedCategory || selectedCategory === "all"))
      ...
      <div className="flex items-center gap-2 border-b border-white/5 pb-5 overflow-x-auto scrollbar-hide w-full max-w-6xl justify-center">
        ...
        {[
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all shrink-0 ${selectedAnimal === filter.id
      ...
      <div className="flex items-center gap-2 border-b border-white/5 py-5 overflow-x-auto scrollbar-hide w-full max-w-6xl justify-center">
        ...
        {[
          ...
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all shrink-0 ${selectedCategory === filter.id
                ? "bg-[#f26422] text-white shadow-md shadow-[#f26422]/10"
                : "bg-[#121212] text-gray-400 hover:bg-[#161616] border border-white/5"
              }`}
          >
            ...
            fiteredProducts.map((prod) => {
              ...
              return (
                <div
                  ...
                  className={`p-6 rounded-3xl border transition-all flex items-center justify-between cursor-pointer group min-h-32 w-full ${isSelected
            ...
            {product ? (
              <div className="flex flex-col gap-3 text-xs font-medium text-gray-300">
                ...
                <div className="flex flex-col gap-1">
                  <span className="text-gray-500 text-[10px] uppercase">Animal</span>
                  <span className="text-sm bg-[#121212] p-2 rounded-lg border border-white/5 capitalize">{product?.animal}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-gray-500 text-[10px] uppercase">Categoria</span>
                  <span className="text-sm bg-[#121212] p-2 rounded-lg border border-white/5 capitalize">{product?.categoria}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-gray-500 text-[10px] uppercase">Nome</span>
                  <span className="text-sm bg-[#121212] p-2 rounded-lg border border-white/5">{product?.label}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-gray-500 text-[10px] uppercase">Descrição</span>
                  <span className="text-sm bg-[#121212] p-2 rounded-lg border border-white/5">{product?.description}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <span className="text-gray-500 text-[10px] uppercase">Preço</span>
                  <span className="text-sm bg-[#121212] p-2 rounded-lg border border-white/5 capitalize">{product?.price.toLocaleString("pt-BR", {
              ...
              <div className="text-center text-gray-500 my-auto py-6 text-xs font-medium">
                Selecione um produto na lista para visualizar as informações aqui.
              </div>
            )}
          </div>
        </div>
