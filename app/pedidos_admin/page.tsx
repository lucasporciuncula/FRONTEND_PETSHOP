...
import { Flame, Clock, LogOut, X, UserIcon, Calendar } from "lucide-react";
import { useAuthContext } from "../context/AuthContext";
import { Order, useCart } from "../hooks/useOrders";
...
export default function AdminPanel() {
  ...
  useEffect(() => {
    console.log("Pedidos reais:", orders);
    console.log("Pedido selecionado atual:", selectedOrder);
  ...
  const nextStatus = async () => {
    ...
    if (selectedOrder.status === "PROCESSANDO") {
      await updateOrderStatus(selectedOrder.id, "ENVIADO");
    }

    if (selectedOrder.status === "ENVIADO") {
      await updateOrderStatus(selectedOrder.id, "ENTREGUE");
    }
          ...
          <div className="flex items-center gap-2 border-b border-[#664533] pb-5 overflow-x-auto scrollbar-hide">
            ...
            {[
              ...
              <button
                ...
                className={`px-4 py-2.5 rounded-xl text-xs font-bold transition-all shrink-0 ${selectedStatusFilter === filter.id
              ...
              filteredOrders.map((order) => {
                ...
                return (
                  <div
                    ...
                    className={`p-7 rounded-4xl border transition-all flex items-center justify-between cursor-pointer group min-h-30 ${isSelected
          ...
          {selectedOrder ? (
            <>
              ...
              <div className="flex flex-col gap-1 bg-[#FAFAF8] border border-[#664533] p-4 rounded-2xl shadow-xl">
                <span className="text-[10px] font-bold text-black uppercase tracking-widest">Tipo de Pedido</span>
                <div className="flex justify-between items-center">
                  {/* <span className="text-xl font-black">{selectedOrder.id}</span> */}
                  <span className="font-extrabold text-lg">
                    {selectedOrder.isDelivery ? "Pedido Delivery" : "Pedido para Retirar"}
                  </span>
                </div>
              </div>
              <div className="flex flex-col gap-3  shadow-xl">
                <div className="flex flex-col gap-3 text-xs font-medium text-black">
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-black font-bold">{selectedOrder.createdAt.slice(0, 10)}</span>
                  </div>
                  <div className="flex items-center gap-2.5">
                    <Clock size={15} className="text-black" />
                    <span className="text-black font-bold">{selectedOrder.createdAt.slice(11, 19)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-auto flex flex-col gap- bg-[#FAFAF8] border border-[#664533] p-4 rounded-2xl text-xs font-medium text-black">

                <div className="flex justify-between items-center border-[#664533] text-black font-black text-sm">
                ...
                <div className="flex flex-3">
                  <button
                    onClick={()=>{
                      nextStatus()
                      setIsConfirmando(!isConfirmando)
                    }}
                    disabled={selectedOrder.status === "ENTREGUE"}
                    className="w-full text-white text-xl border border-[#664533] rounded-2xl bg-[#664533] hover:bg-white hover:text-[#664533] disabled:bg-gray-400 disabled:cursor-not-allowed py-3"
                  >
                    {selectedOrder.status === "PROCESSANDO" && !isConfirmando && "Marcar como Enviado"}
                    {selectedOrder.status === "PROCESSANDO" && isConfirmando && "Confirmar como Enviado"}
                    {selectedOrder.status === "ENVIADO" && !isConfirmando && "Marcar como Entregue"}
                    {selectedOrder.status === "ENVIADO" && isConfirmando && "Confirmar como Entregue"}
                    {selectedOrder.status === "ENTREGUE" && !isConfirmando && "Pedido Finalizado"}
                    {selectedOrder.status === "ENTREGUE" && isConfirmando && "Pedido Finalizado"}
